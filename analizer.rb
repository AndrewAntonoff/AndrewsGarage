...
class ApacheLogAnalyzer
  ...
  def analyze(file_name)
    ...
    File.open(file_name).each do |line|
	   ip=(ip_regex.match(line)).to_s
	    if @total_hits_by_ip[ip]
         ...
         @total_hits_by_ip[ip] = Hash.new(0)
         @total_hits_by_ip[ip] =1
		end 
	   url=(url_regex.match(line)).to_s
	    if @total_hits_per_url[url]
         ...
         @total_hits_per_url[url] = Hash.new(0)
         @total_hits_per_url[url] =1
		end 
	if line.include?"secret"
	   secret=(ip_regex.match(line)).to_s
	    if @secret_hits_by_ip[secret]
	     @secret_hits_by_ip[secret]+=1
		else
         @secret_hits_by_ip[secret] = Hash.new(0)
         @secret_hits_by_ip[secret] =1
		end 
	end	
	...
	@total_hits_by_ip.delete("")
	# @secret_hits_by_ip.delete("")
